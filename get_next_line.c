...
int		check_tail(char **line, char **s_tail, char **find_end, char **array)
{
	...
	if ((*s_tail) != NULL && (*find_end = ft_strchr(*s_tail, '\n')))
	{
		*(*find_end) = '\0';
		...
		cur_ptr = *s_tail;
		if (!(*s_tail = ft_strdup(*find_end + 1)) && ft_error(&cur_ptr, NULL))
			return (-1); // перенести очищение сюда
		ft_error(&cur_ptr, NULL);
		return (1);
	}
	else if (*s_tail != NULL)
	{
...
int		get_next_line(int fd, char **line)
{
	...
	if (fd < 0 || line == NULL || read(fd, NULL, 0) < 0 || BUFFER_SIZE <= 0)
		return (-1);
	find_end = NULL;
